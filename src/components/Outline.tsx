...
const OutlineShaderMaterial = shaderMaterial(
  ...
  `
    void main() {
      vec3 thickenedPosition = position + normal * 0.0;
...
export const Outline: React.FC<OutlineProps> = ({
  ...
  // Clone the children and apply the outline material
  const clonedChildren = React.isValidElement(children)
    ? React.cloneElement(children as React.ReactElement, {
        material: new OutlineShaderMaterial({
    ...
    console.log("Original element", children);
    console.log("Clooned element", clonedChildren);
